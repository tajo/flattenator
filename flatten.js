module.exports = configureFlatten

function configureFlatten(separator, separatorArrayLeft, separatorArrayRight) {
  separator = separator || '.';
    ...
    function _route(prefix, value) {
      ...
      if (typeof value == "object") {
        ...
        if (Array.isArray(value)) {
          len = value.length
          if (len == 0) _route(prefix + separatorArrayLeft + separatorArrayRight, null)
          for (i = 0; i < len; i++) {
            _route(prefix + separatorArrayLeft + i + separatorArrayRight, value[i])
          }
          return
        }
        keys = Object.keys(value)
        len = keys.length
        if (prefix) prefix = prefix + separator
